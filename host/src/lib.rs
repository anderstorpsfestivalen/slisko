...
use engine::controller::{Controller, PATTERN_NAMES};
...
fn run_with(args: Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut controller = Controller::new(chassi, Shaper::new(config::SHAPER), args.seed);
        ...
        for &name in PATTERN_NAMES {
            let make_controller = || {
                let mut controller = Controller::new(
                    Chassi::from_specs(config::CHASSIS),
                    Shaper::new(config::SHAPER),
                    DEFAULT_SEED,
                );
