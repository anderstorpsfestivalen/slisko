...
/// The result of trying to acquire a surface texture for a new frame.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum FrameAcquireStatus {
    /// A surface texture was acquired and the frame may be drawn.
    Acquired,
    /// The surface timed out. Skip this frame and retry later.
    Timeout,
    /// The surface is occluded. Skip frames until the window is exposed again.
    Occluded,
}
...
impl GraphicsContext {
    ...
    pub fn begin_frame(&mut self) -> GameResult<FrameAcquireStatus> {
        if self.fcx.is_some() {
        ...
        let frame = loop {
            match self.surface.get_current_texture() {
                ...
                wgpu::CurrentSurfaceTexture::Timeout => return Ok(FrameAcquireStatus::Timeout),
                wgpu::CurrentSurfaceTexture::Occluded => return Ok(FrameAcquireStatus::Occluded),
